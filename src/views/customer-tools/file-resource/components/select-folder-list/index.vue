 ...
 * @LastEditors  : ADI
 * @LastEditTime : 2021-06-15 10:29:51
-->
<template>
  <div class="selectFolderList tshu_select">
    <div class="tshu_select_input" :class="bindClass" @click="showSelect">
      <svg class="icon folderIcon" aria-hidden="true" v-if="showInputFolderIcon">
        <use xlink:href="#icon-00icon-qiyeweixin-wenjianjiaxiao"></use>
      </svg>
...
export default {
  ...
  props: {
    ...
    isMutil: {
      // 是否包括企业文件夹和我的文件夹，默认包括，true：包括 false：只显示企业文件夹
      type: Boolean,
      default: true,
    },
  },
  data() {
    return {
      selectVal: '企业文件夹', // 选中的文件夹值 默认为企业文件
      folderList: [], // 文件夹层级列表
      isShow: false, // 是否显示下拉框
      placeholder: '请选择文件夹',
      selectId: 0, // 选中的id 0:企业文件
      expandId: [0], // 默认展开的父节点 0:企业文件
      defaultProps: {
        // 层级列表配置
        children: 'child',
        label: 'name',
  ...
  methods: {
    ...
    async getFolderList() {
      ...
      if (this.isMutil) {
        ...
        this.folderList = [res.data.corpMaterial];
      }
