...
import { confirm, getFileName, loading } from '@/utils';
...
export default {
  ...
  methods: {
     ...
     */
    uploadError(file) {
      const tmpUploadFileList = this.uploadFileList;
      this.uploadFileList = []; // 失败后刷新回显列表，不要把失败的文件也显示出来
      this.$nextTick(() => {
        this.uploadFileList = tmpUploadFileList;
      });
    },
    /**
     * 禁止删除
