 ...
 * @LastEditors  : ADI
 * @LastEditTime : 2021-07-02 14:45:07
-->
<template>
  <global-ts-dialog
    ...
    :sure-btn-disabled="disabledSureBtn"
    @cancel="cancelUpload"
    @sure="saveUpload"
  >
              ...
              <div>
                <div :class="['imgBox flex flex-hc flex-vc', { successImgBox: item.success }]">
                  ...
                  <div class="grayMask" v-if="item.success || item.fail" @click="removeFile(item)">
                    <global-ts-svg-icon class="icon" name="icon-icon-10"></global-ts-svg-icon>
                  </div>
                </div>
                <div
...
export default {
  ...
  data() {
    return {
      ...
      requestParam: {
        // 保存接口请求数据
        // isCorp: true, //是否在企业文件夹 true:企业文件夹 false:个人文件夹
  ...
  watch: {
    dialogVisible(newVal) {
      if (newVal) {
        ...
        this.successList = [];
        this.requestParam = {
          isCorp: true, //是否为企业文件夹 true:企业文件夹 false:个人文件夹
  ...
  methods: {
    ...
    showEditName(item, index) {
      if (item.success) {
        ...
        item.oldName = item.editName;
        this.$set(this.showUploadList, index, item);
      }
    },
    /**
    ...
    changeBlur(file, index) {
      ...
      if (!file.editName.length) {
        // 置空则返回原始值
        file.editName = file.oldName;
      }
      this.successList.find(item => {
        if (item.uid == file.uid) {
          const suffix = getFileName(item.name)[1]; // 文件后缀名
          item.name = file.editName + suffix;
          return true;
        }
      });
      this.$set(this.showUploadList, index, file);
    },
    /**
     * 删除文件
     * @author lymn
    ...
    beforeUpload(file) {
      ...
      // if (categoryType == CategoryDef.img) {
      //     if (!this.checkImgBeforeUpload(file)) {
      //         return
      //     }
      // }
