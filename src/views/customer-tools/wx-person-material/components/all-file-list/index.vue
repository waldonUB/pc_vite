 ...
 * @LastEditors  : ADI
 * @LastEditTime : 2021-06-23 10:06:13
-->
<template>
        ...
        <div class="fileList flex" v-if="fileList.length">
          <div v-for="(item, index) in fileList" class="itemBox" :key="index">
            ...
            <div class="fileTitle multiTextEllipsis" @click="showEditName(item, index)">
              ...
              <div :class="['fileName', { canEdit: !item.isSystem }]" v-else>
...
export default {
  ...
  methods: {
    ...
    async addNewGroup() {
      ...
      this.getFileList().then(() => {
        // 新建文件夹后处于编辑态
        const curIndex = this.fileList.findIndex(item => !item.isSystem); // 如果有系统资源文件夹。新创建的文件夹在此文件夹之后
