 ...
 */
export const downloadBlobToImg = function(options) {
  if (window.navigator.msSaveOrOpenBlob) {
    // ie不支持a标签download属性，特殊处理
    const blob = dataURLtoBlob(options.blob);
    window.navigator.msSaveOrOpenBlob(blob, options.fileName);
  } else {
    const a = document.createElement('a'); // 生成一个a元素
    const event = new MouseEvent('click'); // 创建一个单击事件
    a.download = options.fileName; // 设置图片名称
    a.href = options.blob; // 将生成的URL设置为a.href属性
